...
The cost is one cross-repo CI job: `sourcemark-verify` must pass this repository's `conformance/` vectors at a pinned spec tag. That is not overhead so much as a rehearsal — it is precisely what a second implementer will do.
